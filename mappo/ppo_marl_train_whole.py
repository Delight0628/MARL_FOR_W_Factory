...
MAPPO多智能体强化学习训练模块
=================================
实现基于MAPPO算法的工厂调度系统训练，包括：
- 集中式Critic + 分布式Actor架构
- 两阶段渐进式训练策略（基础泛化 → 动态事件强化）
- 课程学习支持
- 自适应熵调整机制
"""
...
class ExperienceBuffer:
    ...
    def get_batch(self, gamma=0.99, lam=0.95, next_value_if_truncated=None, advantage_clip_val: Optional[float] = None):
        ...
        Args:
            gamma: 折扣因子
            lam: GAE平滑参数
            next_value_if_truncated: 截断时的bootstrap价值
            advantage_clip_val: 优势函数裁剪阈值
            
        Returns:
            处理后的批次数据: (states, global_states, actions, action_probs, advantages, returns)
        """
        ...
        for t in reversed(range(len(rewards))):
            ...
            if t == len(rewards) - 1:
                if truncateds[t] and next_value_if_truncated is not None:
                    next_value = next_value_if_truncated  # 截断：使用bootstrap
                elif dones[t]:
                    next_value = 0  # 终止：价值为0
                else:
                    next_value = next_value_if_truncated if next_value_if_truncated is not None else 0
            else:
                if truncateds[t]:
                    next_value = 0  # 中间截断：阻断价值传播
                else:
...
class PPONetwork:
    ...
    def get_action_and_value(self, state: np.ndarray, global_state: np.ndarray) -> Tuple[int, np.float32, np.float32]:
        ...
        Returns:
            (action, value, log_prob): 动作、价值估计、对数概率
            
        注意：
            - MultiDiscrete空间使用无放回采样，避免智能体选择相同候选工件
            - 推理模式运行，禁用Dropout
        """
        action_probs = self.actor(state, training=False)
        value = self.critic(global_state, training=False)

        # MultiDiscrete：多头无放回采样
        if self.is_multidiscrete:
            ...
            for i in range(num_heads):
                ...
                # 采样动作
                sampled = tf.random.categorical(logits, 1)
    ...
    def update(self, states: np.ndarray, global_states: np.ndarray, actions: np.ndarray, 
        ...
        Returns:
            损失统计字典: {actor_loss, critic_loss, entropy, approx_kl, clip_fraction}
        """
...
def run_simulation_worker(network_weights: Dict[str, List[np.ndarray]],
    ...
    """
    并行工作进程：运行仿真环境并收集经验数据
    
    Args:
        network_weights: 网络权重字典 {'actor': weights, 'critic': weights}
        state_dim: 局部观测维度
        action_space: 动作空间对象
        num_steps: 单次仿真最大步数
        seed: 随机种子
    ...
    Returns:
        (buffers, reward, last_values, terminated, graduated): 
        经验缓冲区、总奖励、最终价值、是否终止、是否毕业
    """
    try:
        # ========== GPU/CPU设备配置 ==========
        try:
